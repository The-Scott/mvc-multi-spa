...
const project = require('../aurelia_project/aurelia');
...
// primary config:
const title = 'Multi-SPA-v2:Aurelia';
const outDir = path.resolve(__dirname, project.platform.output);
const srcDir = path.resolve(__dirname, '../Aurelia/src');
...
module.exports = ({ production, server, extractCss, coverage, analyze, karma } = {}) => ({
  ...
  plugins: [
    ...
    ...when(production || server, new CopyWebpackPlugin([
      { from: ('Aurelia/static' ), to: outDir, ignore: ['.*'] }])), // ignore dot (hidden) files
